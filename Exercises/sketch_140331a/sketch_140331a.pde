...
void draw() {
  background(255);
  xCoordinate+=4; // each image loads 4px to right
  if (xCoordinate > width) { // image continues to end of screen
    xCoordinate = -60;
  }
  frame++;
    if (frame == numFrames) { // cycle through images
    frame = 0;
    }
